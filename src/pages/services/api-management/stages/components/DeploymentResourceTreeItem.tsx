  ...
  return (
    <div className="ml-4">
      <div className="flex items-start gap-2 py-1 text-sm">
        <span className="text-gray-900 break-all">{resource.path}</span>
      </div>
        ...
        <div className="ml-4 space-y-1">
          {resource.methods.map((method: any) => (
            <div key={method.id} className="flex items-start gap-2 py-1">
              <span
                className={`${getMethodStyle(method.type)} !font-mono !font-bold !text-xs !px-1.5 !py-0.5 rounded shrink-0`}>
                {method.type}
              </span>
              {method.info?.description && (
                <span className="text-sm text-gray-600 break-words">- {method.info.description}</span>
              )}
            </div>
          ))}
        </div>
