...
export default function EurekaPage() {
  ...
  // 검색 필터링 함수 추가
  const filteredServices = mockEurekaData.services.filter((service) => {
    const searchLower = searchTerm.toLowerCase()
    return (
      service.name.toLowerCase().includes(searchLower) ||
      service.instances.some(
        (instance) =>
          instance.ipAddr.toLowerCase().includes(searchLower) || instance.port.toString().includes(searchLower),
      )
    )
  })
    ...
    <AppLayout>
      <div className="container mx-auto px-4 py-6 space-y-6">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          {/* TabsList 스타일 변경 */}
          <TabsList className="grid w-full grid-cols-2 bg-gradient-to-r from-slate-50 via-blue-50 to-indigo-50 dark:from-slate-900/50 dark:via-blue-900/30 dark:to-indigo-900/50 border-2 border-blue-100 dark:border-blue-800/50 p-1.5 rounded-xl shadow-sm">
            {/* TabsTrigger 스타일 변경 */}
            <TabsTrigger
              value="overview"
              className="flex items-center gap-2 data-[state=active]:bg-gradient-to-r data-[state=active]:from-blue-500 data-[state=active]:to-indigo-600 data-[state=active]:text-white data-[state=active]:shadow-lg data-[state=active]:shadow-blue-200 dark:data-[state=active]:shadow-blue-900/50 data-[state=active]:border-0 text-gray-600 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 transition-all duration-300 rounded-lg font-medium"
            >
          ...
          <TabsContent value="overview" className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-6">
              <Card className="col-span-2 bg-gradient-to-br from-rose-50 to-rose-100 dark:from-rose-900/20 dark:to-rose-800/20 border-rose-200 dark:border-rose-800">
                ...
                <CardContent>
                  <div className="text-2xl font-bold text-rose-700 dark:text-rose-300">ON</div>
            ...
            <Card className="border-blue-200/50 bg-white/70 backdrop-blur-sm dark:bg-[#303C9D1F]">
              <CardContent className="p-6">
                <div className="flex items-center gap-2 mb-4">
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-white">최근 등록/변경된 인스턴스</h3>
                    ...
                    {recentInstances.map((instance, index) => (
                      ...
                      >
                        <div className="col-span-4 flex items-center space-x-2">
                          <span className="font-medium text-gray-900 dark:text-white truncate">{instance.app}</span>
                        </div>
              ...
              <div className="flex items-center gap-3">
                <div className="relative">
                  ...
                  <Input
                    ...
                    className="pl-10 w-80 bg-white/50 dark:bg-gray-800/50 border-gray-200 dark:border-gray-700 focus:border-blue-500 dark:focus:border-blue-400"
                  />
                </div>
                <Button
                  onClick={() => {
        ...
        <Dialog open={!!selectedInstance} onOpenChange={() => setSelectedInstance(null)}>
          <DialogContent ref={modalContentRef} className="max-w-4xl max-h-[80vh] overflow-y-auto rounded-lg">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2">
                <Database className="w-5 h-5" />
                <span>{selectedInstance?.metadata?.team || "서비스"} - 인스턴스 상세 정보</span>
                {selectedInstance && (
            ...
            {selectedInstance && (
              <Card className="p-4">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="font-medium">같은 서비스의 다른 인스턴스</span>
                </div>
                <div className="flex flex-wrap gap-2">
                  {mockEurekaData.services
                    .find((service) =>
